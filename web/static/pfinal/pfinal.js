 ...
 *
 *  <link href="static/pfinal/css/pfinal.css" rel="stylesheet" />
 *  <script src="static/pfinal/pfinal.js"></script>
 *
 * @author  Zou Yiliang
 * @date 20150531
 */

window.pFinal = function () {
};
...
$(function () {
     ...
     * 弹窗提示
     *
     * pFinal.alert("提示信息", callback)
     * pFinal.alert("提示信息", "标题", callback)
     */
    pFinal.alert = function (content, title, success) {
     ...
     * 显示loading图标
     *
     * var load = pFinal.loading();   或者 var load = pFinal.loading(按扭对象或选择器);
     * load.start();
     * setTimeout(function(){load.stop()},3000);
     */
    pFinal.loading = function (btn, content) {
        if (btn) {
            var d = {};
        } else {
        ...
        d.start = function () {
            if (d.btn) {
                ...
                d.btn.html("<span class='pFinal-icon-loading'></span> " + loadingText);
     ...
     * pFinal.toast()
     * pFinal.toast('操作成功')
     * pFinal.toast('操作成功', 3000)
     * pFinal.toast('操作成功',function(){})
     */
    pFinal.toast = function (content, time, callback) {
     ...
     * 确认弹框
     *
     * pFinal.confirm("你确定要执行xxx吗", function(){
     *   alert('执行');
     * },function(){
     *   alert('不执行');
     * });
     ...
     */
    pFinal.confirm = function (content, success, cancel) {
     ...
     */
    pFinal.show = function (selector, callback) {
     ...
     * @param url
     */
    pFinal.ajaxShow = function (url, data) {
        var layerId = "pFinalAjaxPopupLayer";
        var div;
        if ($("#" + layerId).length == 0) {
        ...
        $.get(url, data, function (str) {
            div.html(str);
            this.close = pFinal.show("#" + layerId).close;
        });
        return this;
    }
     ...
     *
     * 示例
     * pFinal.isChinese('我是中文');
     *
     * @param str
     * @returns boolean
     */
    pFinal.isChinese = function (str) {
        var reg = /^[\u4E00-\u9FA5]+$/;
        return reg.test(str);
    }
     ...
     *
     * 示例
     * pFinal.isEmail('123456@qq.com');
     *
     * @param str
     * @returns boolean
     */
    pFinal.isEmail = function (str) {
     ...
     *
     * 示例
     * pFinal.isMobile('13888888888');
     *
     * @param str
     * @returns boolean
     */
    pFinal.isMobile = function (str) {
        var reg = /^1\d{10}$/;
        return reg.test(str);
    }
     ...
     * @returns string
     */
    pFinal.dateFormat = function (date, fmt) {
    ...
    //重要操作的confirm
    //<a href="delete.php" class="pFinal-confirm" data-dialog-content="您确定执行此操作吗">删除</a>
    $(document).on("click", "a.pFinal-confirm", function () {
    ...
    //<input type="checkbox" class="pFinal-select-all" data-selector=":checkbox[name='id[]']" />
    $(".pFinal-select-all").click(function () {
        var _this = this;
    ...
    $(".pFinal-batch").click(function () {
        ...
        var arr = $(".pFinal-batch-item:checked");
